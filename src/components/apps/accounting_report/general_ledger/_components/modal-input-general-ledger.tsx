...
import { setGeneralLedgerData } from '@/store/general_ledger/general_ledger';

import { useGetGeneralLedgerReport } from '@/app/api/hooks/general_ledger/accounting_report/useGetGeneralLedgerReport';
import { inputGeneralLedgerInitialState } from '@/helpers/utils/general_ledger/general_ledger';

interface IModalInputGeneralLedgerProps {
  modal: boolean;
  setModal: (value: boolean) => void;
  refetch?: () => void;
}

const ModalInputGeneralLedger = ({
  modal,
  setModal,
}: IModalInputGeneralLedgerProps) => {
  const isRtl =
    useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl';
  const { mutateAsync: getGeneralLedger } = useGetGeneralLedgerReport();
  const [form, setForm] = useState(inputGeneralLedgerInitialState);
  ...
  const handleCancel = () => {
    if (
      JSON.stringify(form) === JSON.stringify(inputGeneralLedgerInitialState)
    ) {
      setModal(false);
      setForm(inputGeneralLedgerInitialState);
      setEmptyField([]);
    } else {
      Swal.fire({
        title: 'Are you sure?',
  ...
  const handleSubmit = async () => {
    ...
    if (!isMandatoryEmpty) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await getGeneralLedger(form);
            setModal(false);
            setForm(inputGeneralLedgerInitialState);
            setEmptyField([]);

            dispatch(setGeneralLedgerData(response));
...
export default ModalInputGeneralLedger;
