...
import { setIncomeStatementData } from '@/store/general_ledger/income_statement';

import { useGetIncomeStatementReport } from '@/app/api/hooks/general_ledger/accounting_report/useGetIncomeStatementReport';
import { inputIncomeStatementInitialState } from '@/helpers/utils/general_ledger/incomeStatementReport';
interface IModalInputIncomeStatementProps {
  modal: boolean;
  setModal: (value: boolean) => void;
  refetch?: () => void;
}

const ModalInputIncomeStatement = ({
  modal,
  setModal,
}: IModalInputIncomeStatementProps) => {
  const isRtl =
    useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl';
  const { mutateAsync: getIncomeStatement } = useGetIncomeStatementReport();
  const [form, setForm] = useState(inputIncomeStatementInitialState);
  ...
  const handleCancel = () => {
    if (
      JSON.stringify(form) === JSON.stringify(inputIncomeStatementInitialState)
    ) {
      setModal(false);
      setForm(inputIncomeStatementInitialState);
      setEmptyField([]);
    } else {
      Swal.fire({
        title: 'Are you sure?',
  ...
  const handleSubmit = async () => {
    ...
    if (!isMandatoryEmpty) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await getIncomeStatement(form);
            setModal(false);
            setForm(inputIncomeStatementInitialState);
            setEmptyField([]);
            response.periods = form;
            dispatch(setIncomeStatementData(response));
...
export default ModalInputIncomeStatement;
