...
import { setBalanceSheetData } from '@/store/general_ledger/balance_sheet';

import { useGetBalanceSheetReport } from '@/app/api/hooks/general_ledger/accounting_report/useGetBalanceSheetReport';
import { inputBalanceSheetReportInitialState } from '@/helpers/utils/general_ledger/balanceSheetReport';
interface IModalInputBalanceSheetProps {
  modal: boolean;
  setModal: (value: boolean) => void;
  refetch?: () => void;
}

const ModalInputBalanceSheet = ({
  modal,
  setModal,
}: IModalInputBalanceSheetProps) => {
  const isRtl =
    useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl';
  const { mutateAsync: getBalanceSheet } = useGetBalanceSheetReport();
  const [form, setForm] = useState(inputBalanceSheetReportInitialState);
  ...
  const handleCancel = () => {
    if (
      JSON.stringify(form) ===
      JSON.stringify(inputBalanceSheetReportInitialState)
    ) {
      setModal(false);
      setForm(inputBalanceSheetReportInitialState);
      setEmptyField([]);
    } else {
      Swal.fire({
        title: 'Are you sure?',
  ...
  const handleSubmit = async () => {
    ...
    if (!isMandatoryEmpty) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await getBalanceSheet(form);
            setModal(false);
            setForm(inputBalanceSheetReportInitialState);
            setEmptyField([]);
            response.periods = form;
            dispatch(setBalanceSheetData(response));
...
export default ModalInputBalanceSheet;
