...
import { useCreatePKP } from '@/app/api/hooks/hrm/tax_variables/pkp/useCreatePKP';
import { pkpInitialState } from '@/helpers/utils/hrm/tax_variables/pkp';

interface IModalRegisterPKPProps {
  modal: boolean;
  setModal: (value: boolean) => void;
  refetch: () => void;
}
const ModalPKP = ({ modal, setModal, refetch }: IModalRegisterPKPProps) => {
  const { mutateAsync: createAssetCategory } = useCreatePKP();
  const [form, setForm] = useState(pkpInitialState);
  ...
  const handleCancel = () => {
    if (JSON.stringify(form) === JSON.stringify(pkpInitialState)) {
      setModal(false);
      setForm(pkpInitialState);
      setEmptyField([]);
    } else {
      Swal.fire({
        title: 'Are you sure?',
