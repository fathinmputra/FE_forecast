...
const ComponentsAppsNotes = () => {
  const [notesList, setNoteList] = useState<Note[]>([
    ...
    },
    {
      ...
      description:
        'Facilisis curabitur facilisis vel elit sed dapibus sodales purus.',
      date: '11/02/2020',
      isFav: true,
      tag: '',
    },
    {
      ...
      title: 'Download Docs',
      description:
        'Proin a dui malesuada, laoreet mi vel, imperdiet diam quam laoreet.',
      date: '11/04/2020',
      isFav: false,
      tag: 'work',
    },
    ...
    },
    {
      ...
      description:
        'Maecenas condimentum neque mollis, egestas leo ut, gravida.',
      date: '11/09/2020',
      isFav: false,
      tag: '',
    },
  ...
  const [filteredNotesList, setFilteredNotesList] = useState<Note[]>([]);
  ...
  const searchNotes = useCallback(() => {
    if (selectedTab === 'fav') {
      setFilteredNotesList(notesList.filter(d => d.isFav));
    } else if (selectedTab === 'all') {
      setFilteredNotesList(notesList);
    } else {
      setFilteredNotesList(notesList.filter(d => d.tag === selectedTab));
    }
  ...
  const saveNote = () => {
    ...
    if (params.id) {
      const noteIndex = notesList.findIndex(note => note.id === params.id);
      if (noteIndex !== -1) {
      ...
      // Add a new note
      const maxNoteId = notesList.reduce(
        (maxId, note) => Math.max(note.id, maxId),
        0,
      );
      const newNote: Note = {
        id: maxNoteId + 1,
  ...
  const setFav = (note: Note) => {
    const list = [...filteredNotesList];
    const itemIndex = list.findIndex(d => d.id === note.id);
    if (itemIndex !== -1) {
      list[itemIndex] = { ...list[itemIndex], isFav: !list[itemIndex].isFav };
      setFilteredNotesList(list);
      searchNotes();
    }
  };

  const setTag = (note: Note, name = '') => {
    const list = [...filteredNotesList];
    const itemIndex = list.findIndex(d => d.id === note.id);
    if (itemIndex !== -1) {
      list[itemIndex] = { ...list[itemIndex], tag: name };
      setFilteredNotesList(list);
      searchNotes();
    }
  };
  ...
  const editNote = (note: Note | null = null) => {
    setIsShowNoteMenu(false);
    if (note) {
      const noteCopy = { ...note };
      setParams(noteCopy);
  ...
  const deleteNote = () => {
    if (deletedNote !== null) {
      setNoteList(notesList.filter((note: Note) => note.id !== deletedNote.id));
      searchNotes();
  ...
  const showMessage = (
    msg = '',
    type: SweetAlertOptions['icon'] = 'success',
  ) => {
  ...
  const getNoteClass = (tag: string) => {
    switch (tag) {
      case 'personal':
        return 'bg-primary-light shadow-primary';
      case 'work':
        return 'bg-warning-light shadow-warning';
      case 'social':
        return 'bg-info-light shadow-info';
      case 'important':
        return 'bg-danger-light shadow-danger';
      default:
        return 'dark:shadow-dark';
    }
  };
  ...
  const getTagClasses = (tag: string) => {
    switch (tag) {
      case 'personal':
        return { btnClass: 'text-primary', iconClass: 'fill-primary' };
      case 'work':
        return { btnClass: 'text-warning', iconClass: 'fill-warning' };
      case 'social':
        return { btnClass: 'text-info', iconClass: 'fill-info' };
      case 'important':
        return { btnClass: 'text-danger', iconClass: 'fill-danger' };
      default:
  ...
  const getTagButtonClasses = (tag: string) => {
    switch (tag) {
      case 'personal':
        return 'badge badge-outline-primary rounded-3xl capitalize shadow-primary';
      case 'work':
        return 'badge badge-outline-primary rounded-3xl capitalize shadow-warning';
      case 'social':
        return 'badge badge-outline-primary rounded-3xl capitalize shadow-info';
      case 'important':
        return 'badge badge-outline-primary rounded-3xl capitalize shadow-danger';
      default:
        return 'badge badge-outline-primary rounded-3xl capitalize'; // Default class if needed
    }
  };
    ...
    <div>
      <div className='relative flex h-full gap-5 sm:h-[calc(100vh_-_150px)]'>
        <div
          ...
          onKeyDown={e => {
            if (e.key === 'Enter') {
              setIsShowNoteMenu(!isShowNoteMenu);
            }
          }}
        ></div>
        <div
          className={`panel
                    ...
                    xl:relative
                    xl:block
                    xl:h-auto ltr:rounded-r-none
                    ltr:lg:rounded-r-md rtl:rounded-l-none
                    rtl:lg:rounded-l-md ${
                      isShowNoteMenu
        ...
        >
          <div className='flex h-full flex-col pb-16'>
            ...
            <div className='border-white-light my-4 h-px w-full border-b dark:border-[#1b2e4b]'></div>
        ...
        <div className='panel h-full flex-1 overflow-auto'>
          ...
          {filteredNotesList.length ? (
            <div className='min-h-[400px] sm:min-h-[300px]'>
              <div className='grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4'>
                {filteredNotesList.map((note: Note) => {
                  const { btnClass, iconClass } = getTagClasses(note.tag);
                  return (
                    <div
                      className={`panel pb-12 ${getNoteClass(note.tag)}`}
                      key={note.id}
                    >
                            ...
                            >
                              <ul className='text-sm font-medium'>
                                ...
                                <li>
                                  <button
                                    type='button'
                                    onClick={() => deleteNoteConfirm(note)}
                                  >
                      ...
                      <div className='absolute bottom-5 left-0 w-full px-5'>
                        <div className='mt-2 flex items-center justify-between'>
                          <div className='dropdown'>
                            <div className='dropdown'>
                              <Dropdown
                                ...
                                button={
                                  <span>
                                    <IconSquareRotated className={iconClass} />
                                  </span>
                                }
                              >
                                <ul className='text-sm font-medium'>
                          ...
                          <div className='flex items-center'>
                            ...
                            <button
                              type='button'
                              className='text-warning group ltr:ml-2 rtl:mr-2'
                              onClick={() => setFav(note)}
                            >
                              <IconStar
                                className={`h-4.5 w-4.5 group-hover:fill-warning ${
                                  note.isFav && 'fill-warning'
                                }`}
                              />
                            </button>
                          </div>
          ...
          <Transition appear show={isDeleteNoteModal} as={Fragment}>
            <Dialog
              as='div'
              open={isDeleteNoteModal}
              onClose={() => setIsDeleteNoteModal(false)}
              className='relative z-50'
            >
              <Transition.Child
                as={Fragment}
                    ...
                    <Dialog.Panel className='panel dark:text-white-dark w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black'>
                      ...
                      <div className='flex flex-wrap items-center gap-2 bg-[#fbfbfb] py-3 text-lg font-medium ltr:pl-5 ltr:pr-[50px] rtl:pl-[50px] rtl:pr-5 dark:bg-[#121c2c]'>
                        ...
                        {params.tag && (
                          <button
                            type='button'
                            className={getTagButtonClasses(params.tag)}
                          >
                            {params.tag}
                          </button>
                        )}
