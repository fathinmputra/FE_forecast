...
import { useUpdateCashAccount } from '@/app/api/hooks/cash_bank/cash_account/useUpdateCashAccount';

interface IModalUpdateCashAccountProps {
  modal: boolean;
...
const ModalUpdateCashAccount = ({
  modal,
  setModal,
  refetch,
}: IModalUpdateCashAccountProps) => {
  ...
  useEffect(() => {
    if (cashAccountData) {
      setForm({
        status: cashAccountData.result.status || 'active',
        description: cashAccountData.result.description || '',
      });
      setDescriptionLength(cashAccountData.result.description?.length || 0);
    }
  }, [cashAccountData]);
          ...
          <form className='flex min-h-screen items-start justify-center px-4'>
            <Dialog.Panel className='my-8 w-full max-w-4xl overflow-hidden rounded-lg bg-white p-6 shadow-lg'>
              <div className='flex items-center justify-between border-b p-5 text-black'>
                <h5 className='text-lg font-bold'>Update Cash Account</h5>
                <button
...
export default ModalUpdateCashAccount;
