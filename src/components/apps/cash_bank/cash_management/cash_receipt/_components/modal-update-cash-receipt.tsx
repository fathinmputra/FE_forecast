...
import { useUpdateCashReceipt } from '@/app/api/hooks/cash_bank/cash_receipt/useUpdateCashReceipt';

interface IModalUpdateCashReceiptProps {
  modal: boolean;
...
const ModalUpdateCashReceipt = ({
  modal,
  setModal,
  refetch,
}: IModalUpdateCashReceiptProps) => {
  const isRtl =
  ...
  } = useGetCashReceiptByPkid(pkid);
  const updateCashReceipt = useUpdateCashReceipt();
  ...
  useEffect(() => {
    if (cashReceiptData?.result) {
      const receiptDate = new Date(cashReceiptData.result.receipt_date);
      receiptDate.setHours(12, 0, 0, 0); // Set time to noon to avoid timezone issues

      setForm({
        receipt_date: receiptDate.toISOString().split('T')[0],
        additional_notes: cashReceiptData.result.additional_notes || '',
      });
    }
  }, [cashReceiptData]);
  ...
  const handleOnChange = (value: string | Date | null, name: string) => {
    if (name === 'receipt_date' && value instanceof Date) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await updateCashReceipt.mutateAsync({
            data: form,
            pkid,
          });
          ...
          <form className='flex min-h-screen items-start justify-center px-4'>
            <Dialog.Panel className='my-8 w-full max-w-4xl overflow-hidden rounded-lg bg-white p-6 shadow-lg'>
              <div className='flex items-center justify-between border-b p-5 text-black'>
                <h5 className='text-lg font-bold'>Update Cash Receipt</h5>
                <button
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <div>
                      ...
                      <Flatpickr
                        ...
                        onChange={date =>
                          handleOnChange(date[0], 'receipt_date')
                        }
                        value={
                          form.receipt_date
                            ? new Date(form.receipt_date)
                            : undefined
                        }
                      />
                    </div>
...
export default ModalUpdateCashReceipt;
