...
const CashReceiptDetailComponent = ({ data }: ICashReceiptDetail) => {
  if (!data) {
    ...
    <div className='panel border-white-light h-full px-0 pb-10'>
      <div className='mb-5 px-5'>
        ...
        <div className='mb-5 flex flex-col gap-5 md:flex-row md:items-center'>
          ...
          <div className='w-full'>
            <label htmlFor='cash_account_code'>Cash Account Code</label>
            <input
              id='cash_account_code'
              name='cash_account_code'
              type='text'
              placeholder='Cash Account Code'
              className='form-input w-full'
              value={cash_account_code}
              disabled
              style={{ cursor: 'not-allowed' }}
            />
          </div>
        ...
        <div className='mb-5 flex flex-col gap-5 md:flex-row md:items-center'>
          ...
          <div className='w-full'>
            ...
            <input
              ...
              className='form-input w-full'
              value={formatDateDisplay(new Date(receipt_date))}
              disabled
              style={{ cursor: 'not-allowed' }}
            />
          </div>
      ...
      <div className='mb-5 px-5'>
        <h2 className='mb-4 text-xl font-bold'>Receipt Details</h2>
        <div className='flex flex-col gap-5 md:flex-row md:items-center'>
          <div className='w-full'>
            ...
            <input
              ...
              className='form-input w-full'
              value={formatCurrencyDisplay(receipt_amount, 'IDR')}
              disabled
              style={{ cursor: 'not-allowed' }}
            />
          </div>
      ...
      <div className='mt-6 px-5'>
        <h2 className='mb-4 text-xl font-bold'>Receipt Item Details</h2>
        <table className='table-striped'>
          ...
          <tbody>
            {details.map((detail: CashReceiptDetailProperty, index: number) => (
              <tr key={index}>
                <td>{index + 1}</td>
                <td>{detail.account}</td>
                <td>{detail.account_name}</td>
                <td>{formatCurrencyDisplay(detail.account_value, 'IDR')}</td>
                <td>{formatCurrencyDisplay(detail.account_tax || 0, 'IDR')}</td>
                <td>
                  {formatCurrencyDisplay(detail.other_account_fees || 0, 'IDR')}
                </td>
...
export default CashReceiptDetailComponent;
