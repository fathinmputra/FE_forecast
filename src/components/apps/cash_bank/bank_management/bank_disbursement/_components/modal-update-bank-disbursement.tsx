...
import { useUpdateBankDisbursement } from '@/app/api/hooks/cash_bank/bank_disbursement/useUpdateBankDisbursement';

interface IModalUpdateBankDisbursementProps {
  modal: boolean;
...
const ModalUpdateBankDisbursement = ({
  modal,
  setModal,
  refetch,
}: IModalUpdateBankDisbursementProps) => {
  const isRtl =
    useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl';
  const pkid = useSelector((state: IRootState) => state.themeConfig.pkid);
  const {
    data: bankDisbursementData,
    isLoading,
    refetch: refetchDetail,
  } = useGetBankDisbursementByPkid(pkid);
  const updateBankDisbursement = useUpdateBankDisbursement();
  ...
  useEffect(() => {
    if (bankDisbursementData?.result) {
      const disbursementDate = new Date(
        bankDisbursementData.result.disbursement_date,
      );
      ...
      setForm({
        ...
        additional_notes: bankDisbursementData.result.additional_notes || '',
      });
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await updateBankDisbursement.mutateAsync({
            data: form,
            pkid,
          });
          ...
          <form className='flex min-h-screen items-start justify-center px-4'>
            <Dialog.Panel className='my-8 w-full max-w-4xl overflow-hidden rounded-lg bg-white p-6 shadow-lg'>
              <div className='flex items-center justify-between border-b p-5 text-black'>
                <h5 className='text-lg font-bold'>Update Bank Disbursement</h5>
                <button
...
export default ModalUpdateBankDisbursement;
