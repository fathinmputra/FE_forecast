...
import { useCreateBankDisbursement } from '@/app/api/hooks/cash_bank/bank_disbursement/useCreateBankDisbursement';
import { useGetEligiblePurchaseDownPaymentInvoice } from '@/app/api/hooks/cash_bank/bank_disbursement/useGetEligiblePurchaseDownPaymentInvoice';
import { useGetEligiblePurchaseInvoice } from '@/app/api/hooks/cash_bank/bank_disbursement/useGetEligiblePurchaseInvoice';
...
interface IModalCreateBankDisbursementProps {
  modal: boolean;
  setModal: (value: boolean) => void;
  refetch: () => void;
}

const ModalCreateBankDisbursement = ({
  modal,
  setModal,
  refetch,
}: IModalCreateBankDisbursementProps) => {
  ...
  const createBankDisbursement = useCreateBankDisbursement();
  ...
  const [referenceCodeDropdown, setReferenceCodeDropdown] = useState<
    SelectOptionProperty[]
  >([]);
  const [form, setForm] = useState(bankDisbursementInitialState);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await createBankDisbursement.mutateAsync(form);
          setForm(bankDisbursementInitialState);
          setIsLoadingCreate(false);
          setModal(false);
          Swal.fire(
            'Saved!',
            'Your bank disbursement has been saved.',
            'success',
          ).then(() => {
            refetch();
          });
...
export default ModalCreateBankDisbursement;
