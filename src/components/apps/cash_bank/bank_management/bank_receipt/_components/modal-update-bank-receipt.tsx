...
import { useUpdateBankReceipt } from '@/app/api/hooks/cash_bank/bank_receipt/useUpdateBankReceipt';

interface IModalUpdateBankReceiptProps {
  modal: boolean;
...
const ModalUpdateBankReceipt = ({
  modal,
  setModal,
  refetch,
}: IModalUpdateBankReceiptProps) => {
  const isRtl =
  ...
  } = useGetBankReceiptByPkid(pkid);
  const updateBankReceipt = useUpdateBankReceipt();
  ...
  useEffect(() => {
    if (bankReceiptData?.result) {
      const receiptDate = new Date(bankReceiptData.result.receipt_date);
      receiptDate.setHours(12, 0, 0, 0); // Set time to noon to avoid timezone issues

      setForm({
        receipt_date: receiptDate.toISOString().split('T')[0],
        additional_notes: bankReceiptData.result.additional_notes || '',
      });
    }
  }, [bankReceiptData]);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await updateBankReceipt.mutateAsync({
            data: form,
            pkid,
          });
          ...
          <form className='flex min-h-screen items-start justify-center px-4'>
            <Dialog.Panel className='my-8 w-full max-w-4xl overflow-hidden rounded-lg bg-white p-6 shadow-lg'>
              <div className='flex items-center justify-between border-b p-5 text-black'>
                <h5 className='text-lg font-bold'>Update Bank Receipt</h5>
                <button
...
export default ModalUpdateBankReceipt;
