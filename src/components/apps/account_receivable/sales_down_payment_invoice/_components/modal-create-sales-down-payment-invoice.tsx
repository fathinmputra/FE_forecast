...
import { useCreateSalesDownPaymentInvoice } from '@/app/api/hooks/account_receivable/sales_down_payment_invoice/useCreateSalesDownPaymentInvoice';
import { useGetEligibleSalesOrders } from '@/app/api/hooks/account_receivable/sales_down_payment_invoice/useGetEligibleSalesOrders';
import { useGetLatestSalesDownPaymentInvoiceBySalesOrderCode } from '@/app/api/hooks/account_receivable/sales_down_payment_invoice/useGetLatestSalesDownPaymentInvoiceBySalesOrderCode';
...
import {
  Currency,
  DpMethodOption,
  salesDownPaymentInvoiceInitialState,
  SalesOrder,
} from '@/helpers/utils/account_receivable/sales_down_payment_invoice';
...
interface IModalNewSalesDownPaymentInvoiceProps {
  modal: boolean;
  setModal: (value: boolean) => void;
  refetch: () => void;
}

const ModalCreateSalesDownPaymentInvoice = ({
  modal,
  setModal,
  refetch,
}: IModalNewSalesDownPaymentInvoiceProps) => {
  const isRtl =
  ...
  const { data: latestInvoiceData } =
    useGetLatestSalesDownPaymentInvoiceBySalesOrderCode(selectedCode);
  const createSalesDownPaymentInvoice = useCreateSalesDownPaymentInvoice();
  ...
  const [form, setForm] = useState(salesDownPaymentInvoiceInitialState);
  ...
  const formatFormToInvoiceProperty = (
    form: typeof salesDownPaymentInvoiceInitialState,
  ) => {
    return {
      ...form,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await createSalesDownPaymentInvoice.mutateAsync(
            formatFormToInvoiceProperty(form),
          );
          setForm(salesDownPaymentInvoiceInitialState);
          setIsLoadingCreate(false);
          setModal(false);
          Swal.fire(
            'Saved!',
            'Your sales down payment invoice has been saved.',
            'success',
          ).then(() => {
            refetch();
          });
            ...
            <Dialog.Panel className='my-8 w-full max-w-4xl overflow-hidden rounded-lg bg-white p-6 shadow-lg'>
              <div className='flex items-center justify-between border-b p-5 text-black'>
                <h5 className='text-lg font-bold'>
                  Create New Sales Down Payment Invoice
                </h5>
                <button
...
export default ModalCreateSalesDownPaymentInvoice;
