...
import { useUpdatePurchaseInvoice } from '@/app/api/hooks/account_payable/purchase_invoice/useUpdatePurchaseInvoice';
    ...
    if (invoiceData) {
      setForm({
        ...
        due_date: invoiceData.result.due_date
          ...
          : undefined,
        received_date: invoiceData.result.received_date
          ? new Date(invoiceData.result.received_date)
          : undefined,
          ...
          <form className='flex min-h-screen items-start justify-center px-4'>
            <Dialog.Panel className='my-8 w-full max-w-4xl overflow-hidden rounded-lg bg-white p-6 shadow-lg'>
              <div className='flex items-center justify-between border-b p-5 text-black'>
                <h5 className='text-lg font-bold'>Update Purchase Invoice</h5>
                <button
